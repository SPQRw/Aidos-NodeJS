...
export class Router {
  ...
  createRoutes() {
    ...
    this.app.post("/confirm", (req, res) => {
      if (this.controller.mainUserPage(req, res)) {
        this.controller.checkCaptcha(req, res);
      } else {
        this.controller.confirmPage(req, res);
      }
    });

    this.app.post("/confirmed", (req, res) => {
      if (this.controller.mainUserPage(req, res)) {
        this.controller.checkConfirmCode(req, res);
      } else {
        this.controller.redirToUserPage(req, res);
      }
    });
  }
}
